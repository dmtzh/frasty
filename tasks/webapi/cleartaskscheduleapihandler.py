...
def validate_task_id(raw_id_with_checksum: str) -> Result[TaskIdValue, InvalidId]:
    return parse_value(raw_id_with_checksum, "task_id", TaskIdValue.from_value_with_checksum).map_error(lambda _: InvalidId())
...
@coroutine_result()
async def clear_schedule_workflow(
    clear_schedule_handler: Callable[[ClearTaskScheduleCommand], Coroutine[Any, Any, Result]],
    tasks_storage: TasksStore,
    raw_id_with_checksum: str,
    raw_schedule_id_with_checksum: str):
    @async_result
...
async def handle(tasks_storage: TasksStore, raw_id_with_checksum: str, raw_schedule_id_with_checksum: str):
    res = await clear_schedule_workflow(http_request_clear_schedule_handler, tasks_storage, raw_id_with_checksum, raw_schedule_id_with_checksum)
    match res:
