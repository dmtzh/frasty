    ...
    def __call__(self, handler: Callable[[T], Coroutine[Any, Any, Result | None]]):
        ...
        async def definition_completed_subscriber_wrapper(input_res: Result[T, Any]) -> Result | None:
            res = await input_res\
                ...
                .merge()
            return res
        return rabbit_definition_completed.subscriber(rabbit_client, self._input_adapter, queue_name="pending_task_history_results", requeue_chance=RequeueChance.HIGH)(definition_completed_subscriber_wrapper)
