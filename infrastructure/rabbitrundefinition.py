...
class _python_pickle:
    ...
    class decoder():
        ...
        @staticmethod
        def _validate_rabbitmq_parsed_data(input_adapter: Callable[[RunIdValue, DefinitionIdValue, dict], R], rabbit_msg_err: RabbitMessageErrorCreator, parsed_data: tuple[str, str, dict]) -> Result[R, RabbitMessageError]:
            ...
            definition_id_res = parse_value(definition_id_unvalidated, "definition_id", DefinitionIdValue.from_value_with_checksum)
            match run_id_res, definition_id_res:
                case Result(tag=ResultTag.OK, ok=run_id), Result(tag=ResultTag.OK, ok=definition_id):
                    res = input_adapter(run_id, definition_id, metadata_unvalidated)
