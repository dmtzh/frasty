...
class _python_pickle:
    @staticmethod
    def data_to_message(task_id: TaskIdValue, schedule_id: ScheduleIdValue, command_dto: dict) -> PythonPickleMessage:
    ...
    class decoder():
        def __init__(self, input_adapter: Callable[[TaskIdValue, ScheduleIdValue, dict], R]):
        ...
        @staticmethod
        def _validate_rabbitmq_parsed_data(input_adapter: Callable[[TaskIdValue, ScheduleIdValue, dict], R], rabbit_msg_err: RabbitMessageErrorCreator, parsed_data: tuple[str, str, dict]) -> Result[R, RabbitMessageError]:
            task_id_unvalidated, schedule_id_unvalidated, command_dto = parsed_data
            ...
            match task_id_res, schedule_id_res:
                case Result(tag=ResultTag.OK, ok=task_id), Result(tag=ResultTag.OK, ok=schedule_id):
                    res = input_adapter(task_id, schedule_id, command_dto)
                    return Result.Ok(res)
                case _:
                    errors_with_none = [task_id_res.swap().default_value(None), schedule_id_res.swap().default_value(None)]
...
def run(rabbit_client: RabbitMQClient, task_id: TaskIdValue, schedule_id: ScheduleIdValue, command_dto: dict):
    message = _python_pickle.data_to_message(task_id, schedule_id, command_dto)
    return rabbit_client.send_command(CHANGE_TASK_SCHEDULE_COMMAND, message)

class handler:
    def __init__(self, rabbit_client: RabbitMQClient, input_adapter: Callable[[TaskIdValue, ScheduleIdValue, dict], R]):
